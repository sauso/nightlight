...
### Added
- The morning review now also asks about an out-of-bed event where the bed went on to show
  movement in several separate minutes with no return logged ("Still moving?"), separately from the
  collapsed full list — never duplicating the existing quick check-in prompt when both apply to the
  same event.
