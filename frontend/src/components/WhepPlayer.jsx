...
import { useNowPlayingSession, NOW_PLAYING_ARTWORK } from '../lib/useNowPlaying.js';

// Re-exported for backwards compatibility with any importer that reached for it here; the
// definition now lives with the shared media-session hook, since both players use it.
export { NOW_PLAYING_ARTWORK };
...
export default function WhepPlayer({
  ...
  // last one to connect/reconnect won, showing the wrong camera). Shared with HlsPlayer so
  // Compatibility mode's iOS background audio shows the same name/artwork/controls.
  useNowPlayingSession({
    enabled: isBackgroundAudio && state === 'live',
    cameraName,
    mediaElRef: audioRef,
  });
